class ReservationsController < ApplicationController
  ...
  def make_actions
    ...
    params.each do |check|
      ...
      if    (check[1] == "MakeRes"    ) then make_res(check[0].to_i, sync_calendar) # Prenotare  uno spazio
      elsif (check[1] == "AddFavSp"   ) then add_fav_sp((check[0].to_i)*(-1))       # Aggiungere uno spazio ai  preferiti
      elsif (check[1] == "RmFavSp"    ) then rm_fav_sp((check[0].to_i)*(-1))        # Rimuovere  uno spazio dai preferiti
      elsif (check[1] == "SetQkRes"   ) then set_qk_res(qk_parse(check[0]))         # Impostare  uno spazio come  prenotazione rapida
      elsif (check[1] == "UpdateQkRes") then update_qk_res(qk_parse(check[0]))      # Sostituire uno spazio alla  prenotazione rapida
      elsif (check[1] == "RmQkRes"    ) then rm_qk_res(qk_parse(check[0]))          # Rimuovere  lo  spazio dalla prenotazione rapida
      end
    end
